...
if ( ! function_exists( 'winter_breadcrumbs' ) ) {
	function winter_breadcrumbs( $args = array() ) {
		if ( is_front_page() ) {
			return;
		}
		...
		$defaults = array(
			...
			'home_title'          => esc_html__( 'Home', 'winter' ),
		);
		$args     = apply_filters( 'winter_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
		...
		*/
		...
		$html .= '<a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url( '/' ) ) . '" title="' . esc_attr( $args['home_title'] ) . '">' . esc_html__( 'Home', 'winter' ) . ' <i class="fa-solid fa-angle-right m-l-8 m-r-9" aria-hidden="true"></i></a>';
		// Post
		if ( is_singular( 'post' ) ) {
			...
			$html .= '<span class="s-text17 bread-search">' . esc_html__( 'Search results for:', 'winter' ) . get_search_query() . '</span>';
		} elseif ( is_404() ) {
			$html .= '<span>' . esc_html__( 'Error 404', 'winter' ) . '</span>';
		} elseif ( is_home() ) {
			$html .= '<span class="s-text17">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
		}
		$html .= '</div>';
		$html  = apply_filters( 'winter_breadcrumbs_filter', $html );

		if ( winter_is_wc_activated() && is_shop() ) {
